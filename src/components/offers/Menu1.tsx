...
const Menu1: React.FC = () => {
  ...
  const [selectedSubmenuItems, setSelectedSubmenuItems] = useState<Set<string>>(
    new Set()
  );
  ...
  // Function to toggle submenu item selection
  const toggleSubmenuItem = (menuId: number, submenuIndex: number) => {
    const submenuKey = `${menuId}-${submenuIndex}`;
    if (selectedSubmenuItems.has(submenuKey)) {
      selectedSubmenuItems.delete(submenuKey);
    } else {
      selectedSubmenuItems.add(submenuKey);
    }
    setSelectedSubmenuItems(new Set(selectedSubmenuItems));
  };

  // Function to check if a submenu item is selected
  const isSubmenuItemChecked = (
    menuId: number,
    submenuIndex: number
  ): boolean => {
    return selectedSubmenuItems.has(`${menuId}-${submenuIndex}`);
  };
          ...
          <div className="flex flex-col justify-around gap-y-5 lg:flex-row lg:gap-y-0">
            ...
            {menus.map((menu, index) => (
              <div key={menu.id} className="relative mr-4">
                <button
                  className="flex items-center gap-2 text-lg font-medium text-white lg:text-base"
                      ...
                      (submenu, subIndex) => (
                        <div key={subIndex} className="px-4 py-2">
                          <div
                            ...
                            onClick={() =>
                              toggleSubmenuItem(menu.id, subIndex)
                            }>
                            <div className="flex space-x-2.5">
                              <input
                                type="checkbox"
                                checked={isSubmenuItemChecked(
                                  menu.id,
                                  subIndex
                                )}
