...
import InputField from '../common/Forms/InputFieldTrackingSoftware';
...
const schema = z.object({
  ...
  program_type: z
    .string()
    .regex(/Tracking Software/, 'Program type must be Tracking Software'),
  ...
  payment_frequency: z.enum(
    ['One time', 'Daily', 'Weekly', 'Monthly', 'Quarterly', 'Annually'],
    {
      errorMap: (_issue, _ctx) => ({
        message:
          'Must be one of the following: One time, Daily, Weekly, Monthly, Quarterly, Annually',
      }),
    }
  ),
...
export type TrackingSoftwareFormData = z.infer<typeof schema>;

const SignUpFormTrackingSoftware = () => {
  const {
    register,
    handleSubmit,
    control,
    formState: { errors },
  } = useForm<TrackingSoftwareFormData>({ resolver: zodResolver(schema) });
  const {
    mutate,
    isLoading,
    isError,
    data: response,
  } = usePost<TrackingSoftwareFormData>('top-it');
  ...
  const onSubmit = ({
    ...
    mutate({
      ...
      unlimited_offers,
      users,
      program_type: 'Tracking Software',
    });
    // console.log(mutate);
    ...
    <>
      ...
      <form className="my-12" onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="space-y-4">
          ...
          <InputField
            label="Tags (CpaNetwork, AffiliateNetwork, MarketingNetwork)"
            id="tag"
            placeholder=""
            type="text"
            register={register}
          ...
          <div>
            <label
              htmlFor="unlimited_offers"
              className="mt-4 block text-base font-bold leading-relaxed text-zinc-800">
              Unlimited Offers
            </label>
            <select
              {...register('unlimited_offers')}
              className="h-[37.07px] w-full bg-stone-100 text-center text-xl font-normal text-zinc-800">
            ...
            {errors.unlimited_offers && (
              <div className="text-red-500">
                {errors.unlimited_offers?.message}
              </div>
            )}
          </div>
          ...
          <div>
            ...
            {errors.setup_fees && (
              <div className="text-red-500">{errors.setup_fees?.message}</div>
            )}
          </div>
          ...
          <InputField
            label="Startup Pricing ($50, $100, more)"
            id="startup_pricing"
            placeholder=""
            type="number"
            register={register}
          ...
          <InputField
            label="Payment Frequency (Weekly, Monthly, Yearly, One-Time Purchase)"
...
export default SignUpFormTrackingSoftware;
