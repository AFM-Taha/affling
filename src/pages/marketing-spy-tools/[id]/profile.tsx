...
export interface FetchedMarketingSpyTools {
  success: boolean;
  data: Data;
}
...
function AffiliateProgramProfile() {
  ...
  const { data, isLoading, isError, error } = useGet<FetchedMarketingSpyTools>(
    `top-it/${query.id}`
  );
          ...
          <div className="">
            ...
            <div className="max-w-[872px]">
              ...
              <div className="mb-5 flex items-center gap-x-3 bg-neutral-200 text-sm font-normal leading-relaxed text-black">
                <p className="w-72">Customer Manager</p>
                <div>
                  {data.data.publishers_contact.map((item) => {
                    return (
