...
const AffiliateNetworkAdmin: React.FC = () => {
  ...
  // Fetch All Data
  // const [networkData, setNetworkData] = useState<AffiliateDataType[]>([]);
  // useEffect(() => {
  ...
  const activeCount = data.filter((el: any) => el.status === 'Active').length;
  const sponsorCount = data.filter((el: any) => el.status === 'Sponsor').length;
